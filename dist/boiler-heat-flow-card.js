class BoilerHeatFlowCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    this._config = {
      ...
      collector: { label: 'Zonnecollector', icon: 'mdi:white-balance-sunny', entity: '', pump: '' },
      hotwater: { label: 'Tapwater', icon: 'mdi:water-boiler', entity: '', active: '' },
      fireplace: { label: 'Openhaard', icon: 'mdi:fireplace', entity: '', active: '' },
      heatpump: { label: 'Warmtepomp', icon: 'mdi:heat-pump', entity: '', active: '' },
      heating: { label: 'Verwarming', icon: 'mdi:radiator', entity: '', active: '' },
      thresholds: {
        collector_delta: 5,
        fireplace_temp: 45,
        heatpump_temp: 30,
      ...
      collector: { label: 'Zonnecollector', icon: 'mdi:white-balance-sunny', entity: '', pump: '', ...(config.collector || {}) },
      hotwater: { label: 'Tapwater', icon: 'mdi:water-boiler', entity: '', active: '', ...(config.hotwater || {}) },
      fireplace: { label: 'Openhaard', icon: 'mdi:fireplace', entity: '', active: '', ...(config.fireplace || {}) },
      heatpump: { label: 'Warmtepomp', icon: 'mdi:heat-pump', entity: '', active: '', ...(config.heatpump || {}) },
      heating: { label: 'Verwarming', icon: 'mdi:radiator', entity: '', active: '', ...(config.heating || {}) },
      thresholds: {
        collector_delta: 5,
        fireplace_temp: 45,
        heatpump_temp: 30,
  ...
  _isOn(entityId) {
    ...
    return ['on', 'home', 'heat', 'heating', 'true', 'open', 'active'].includes(value);
  }
  ...
  _activeFromConfig(sectionName, thresholdKey) {
  ...
  _render() {
    ...
    const hotwaterActive = this._activeFromConfig('hotwater', 'hotwater_temp');
    const fireplaceActive = this._activeFromConfig('fireplace', 'fireplace_temp');
    const heatpumpActive = this._activeFromConfig('heatpump', 'heatpump_temp');
    const heatingActive = this._activeFromConfig('heating', 'heating_temp');
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .boiler-overlay {
          position: absolute;
          inset: 0;
          background: linear-gradient(180deg, rgba(255,255,255,0.2), rgba(255,255,255,0.02) 35%, rgba(255,255,255,0.01));
          pointer-events: none;
        }
        .boiler-title {
          position: absolute;
      ...
      <ha-card>
        <div class="wrap">
          ...
          <svg class="flow" viewBox="0 0 100 100" preserveAspectRatio="none">
            ...
            <path class="pipe-bg" d="M77 76 L61 76 Q55 76 55 70 L55 62" />
            <path class="pipe-glow" d="M77 76 L61 76 Q55 76 55 70 L55 62" />
            <path class="pipe-active pipe-hot ${heatingActive && this._config.animations ? 'on' : ''}" d="M77 76 L61 76 Q55 76 55 70 L55 62" />
          </svg>
          ...
          ${this._nodeTemplate('heating', this._config.heating, 86, 76, heatingActive)}
...
class BoilerHeatFlowCardEditor extends HTMLElement {
  ...
  _entityOptions(domainFilter = null) {
    ...
    const filtered = domainFilter ? ids.filter((id) => id.startsWith(`${domainFilter}.`)) : ids;
    ...
    return `
      <label class="field">
        ...
        <select data-path="${path}">
          ${options.map((option) => `<option value="${option}" ${String(option) === String(value) ? 'selected' : ''}>${option || '— niet ingesteld —'}</option>`).join('')}
        </select>
  ...
  _render() {
    ...
    const sensorOptions = this._entityOptions('sensor');
    const binaryOptions = this._entityOptions('binary_sensor');
    const allOptions = this._entityOptions(null);
    ...
    this.shadowRoot.innerHTML = `
      ...
      ${this._renderSection('Algemeen', `
        ...
        ${this._renderSelect('Animaties', 'animations', ['', 'true', 'false'], 'Laat leeg voor standaard true')}
      `)}

      ${this._renderSection('Boiler', `
        ${this._renderInput('Titel boiler', 'tank.title')}
        ${this._renderSelect('Boiler boven', 'tank.top', sensorOptions)}
        ${this._renderSelect('Boiler midden', 'tank.middle', sensorOptions)}
        ${this._renderSelect('Boiler onder', 'tank.bottom', sensorOptions)}
      `)}

      ${this._renderSection('Zonnecollector', `
        ${this._renderSelect('Temperatuur entity', 'collector.entity', sensorOptions)}
        ${this._renderSelect('Pomp / actief entity', 'collector.pump', binaryOptions, 'Optioneel. Zonder pomp gebruikt de kaart delta met boiler boven.')}
        ${this._renderInput('Label', 'collector.label')}
        ${this._renderInput('Icoon', 'collector.icon')}
      `)}

      ${this._renderSection('Tapwater', `
        ${this._renderSelect('Temperatuur entity', 'hotwater.entity', sensorOptions)}
        ${this._renderSelect('Actief entity', 'hotwater.active', binaryOptions, 'Optioneel. Anders gebruikt de kaart de temperatuur-drempel.')}
        ${this._renderInput('Label', 'hotwater.label')}
        ${this._renderInput('Icoon', 'hotwater.icon')}
      `)}

      ${this._renderSection('Openhaard', `
        ${this._renderSelect('Temperatuur entity', 'fireplace.entity', sensorOptions)}
        ${this._renderSelect('Actief entity', 'fireplace.active', binaryOptions)}
        ${this._renderInput('Label', 'fireplace.label')}
        ${this._renderInput('Icoon', 'fireplace.icon')}
      `)}

      ${this._renderSection('Warmtepomp', `
        ${this._renderSelect('Temperatuur entity', 'heatpump.entity', sensorOptions)}
        ${this._renderSelect('Actief entity', 'heatpump.active', binaryOptions)}
        ${this._renderInput('Label', 'heatpump.label')}
        ${this._renderInput('Icoon', 'heatpump.icon')}
      `)}

      ${this._renderSection('Verwarming', `
        ${this._renderSelect('Temperatuur entity', 'heating.entity', sensorOptions)}
        ${this._renderSelect('Actief entity', 'heating.active', binaryOptions)}
      ...
      ${this._renderSection('Drempels', `
        ...
        ${this._renderInput('Verwarming temperatuur', 'thresholds.heating_temp', 'number')}
